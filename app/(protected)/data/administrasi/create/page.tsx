...
export default function CreateAdministrasiPage() {
  ...
  const getMonthName = (monthNum: string): string => {
    if (!monthNum) return "";
    return new Date(0, parseInt(monthNum) - 1).toLocaleString('id-ID', { month: 'long' });
  };
                ...
                {type === "kepatuhan" && (
                  <>
                    <div className="w-full">
                        ...
                        <Input value={form.indikator} onChange={(e) => handleChange(index, "indikator", e.target.value)} placeholder="Indikator Kepatuhan" className="w-full" />
                    </div>
                     <div className="w-full">
                        <Label className="mb-1 block">Kategori</Label>
                        <Input value={form.kategori} onChange={(e) => handleChange(index, "kategori", e.target.value)} placeholder="Kategori" className="w-full" />
                    </div>
                    <div className="w-full">
                        <Label className="mb-1 block">Target</Label>
